...
NeuroForge is an intelligent learning platform that combines an AI Mentor (powered by LLMs), a Knowledge Graph (Neo4j), cognitive profiling, and structured learning pathways to deliver personalized education experiences.
...
NeuroForge is built as a **microservices architecture** with 7 backend services, an API gateway, and a React frontend — all orchestrated via Docker Compose.
...
└──┬───┘└──┬───┘└──┬───┘└──────┘└──────┘└──┬───┘└──┬───┘
   ...
   ▼       ▼       ▼                        ▼       ▼
┌──────┐┌──────┐┌──────┐              ┌──────┐┌──────┐
│Postgr││Neo4j ││OpenAI│              │Postgr││Neo4j │
│  SQL ││      ││ API  │              │  SQL ││      │
└──────┘└──────┘└──────┘              └──────┘└──────┘
```
...
| **AI Mentor Service** | 8003 | LLM-powered tutoring with KG context |
...
- An [OpenAI API key](https://platform.openai.com/api-keys) (for the AI Mentor)
...
### AI Mentor
```
POST /api/v1/mentor/query        — Send a question to the AI Mentor
...
│           ├── MentorChat.js       # AI chat with personas
...
│   ├── ai_mentor_service/          # LLM + KG integration
...
- ✅ AI Mentor with OpenAI integration, KG context injection, and 4 personas
...
- 🔲 Vector database (Pinecone/Weaviate) for AI Mentor long-term memory
